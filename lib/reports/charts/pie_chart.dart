...
import 'package:base_tools/layout/letter_spacing.dart';
  ...
  return List<RallyPieChartSegment>.generate(
    ...
    (i) {
      return RallyPieChartSegment(
        color: ReportColors.accountColor(i),
...
List<RallyPieChartSegment> buildSegmentsFromBillItems(List<BillData> items) {
  return List<RallyPieChartSegment>.generate(
    ...
    (i) {
      return RallyPieChartSegment(
        color: ReportColors.billColor(i),
        value: items[i].primaryAmount,
      );
    },
  );
...
List<RallyPieChartSegment> buildSegmentsFromBudgetItems(
  ...
  return List<RallyPieChartSegment>.generate(
    ...
    (i) {
      return RallyPieChartSegment(
        color: ReportColors.budgetColor(i),
...
List<RallyPieChartSegment> buildSegmentsFromSalesProductItems(
    List<SalesProductData> items) {
  return List<RallyPieChartSegment>.generate(
    items.length,
        (accountDataIndex) {
      return RallyPieChartSegment(
        color: ReportColors.budgetColor(accountDataIndex),
        value: items[accountDataIndex].amount,
      );
    },
  );
}
...
class _AnimatedRallyPieChart extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      if (MovileOptions.of(context).textScaleFactor(context) > 1.0) {
...
class _RallyPieChartOutlineBoxPainter extends BoxPainter {
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration configuration) {
    ...
    // display as segments.
    final bgPaint = Paint()..color = ReportColors.primaryBackground;
